...
import { generateCharacterImage } from "@/lib/tse/pov-image";
...
export async function POST(_: Request, { params }: { params: Promise<{ sessionId: string }> }) {
  ...
  const { data: session } = await admin.from("training_sessions").select("user_id, difficulty, image_path, public_briefing").eq("id", sessionId).maybeSingle();
  if (!session || session.user_id !== user.id) return NextResponse.json({ error: "Ocorrência indisponível." }, { status: 404 });
  if (session.image_path) return NextResponse.json({ scene: true, generated: false });
  ...
  const jobs: Promise<{ field: "image_path"; path: string }>[] = [
    (async () => {
      const path = sessionId + "/scene.webp";
  ...
  const scene = Boolean(session.image_path || updates.image_path);
  if (!scene) {
  ...
  return NextResponse.json({ scene, generated: true });
}
