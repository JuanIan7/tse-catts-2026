...
export async function GET(request: Request, { params }: { params: Promise<{ sessionId: string }> }) {
  ...
  if (!turn || turn.speaker !== "PERSONAGEM" || turn.delivery_status !== "PENDENTE") return NextResponse.json({ error: "Resposta de voz indisponível." }, { status: 409 });
  if (!process.env.OPENAI_API_KEY) return NextResponse.json({ error: "Integração de voz indisponível." }, { status: 503 });
  const response = await fetch("https://api.openai.com/v1/audio/speech", { method: "POST", headers: { Authorization: `Bearer ${process.env.OPENAI_API_KEY}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: "gpt-4o-mini-tts", voice: "marin", input: turn.content, response_format: "mp3" }) });
  if (!response.ok) return NextResponse.json({ error: "Não foi possível gerar a voz do tentante." }, { status: 502 });
