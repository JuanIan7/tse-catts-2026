...
type SessionRow = { id: string; difficulty: string; status: string; image_path: string | null; public_briefing: PublicBriefing };
...
export default async function SessionPage({ params }: { params: Promise<{ sessionId: string }> }) {
  try {
    ...
    const { data, error } = await supabase.from("training_sessions").select("id, difficulty, status, image_path, public_briefing").eq("id", sessionId).maybeSingle();
