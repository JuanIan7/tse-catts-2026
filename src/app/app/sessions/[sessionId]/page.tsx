...
import { endTrainingSession } from "@/app/app/actions";
...
import { remainingSessionMs } from "@/lib/tse/active-session-clock";
import type { Difficulty } from "@/lib/tse/session-case";

type SessionRow = { id: string; difficulty: Difficulty; status: string; image_path: string | null; public_briefing: unknown; started_at: string | null; active_elapsed_ms: number; active_activity: "PAUSED" | "VOICE_STUDENT" | "VOICE_CHARACTER" | "TEXT"; active_started_at: string | null };
...
type Evaluation = { result: string; final_score: number; partial: boolean; calculation: { cobertura?: { avaliados: number; total: number; percentual: number }; rotulo?: string; aviso_parcial?: string | null; itens?: EvaluationItem[]; erros_graves?: { titulo?: string; evidencia?: string; ajuste?: number; aplicado?: boolean }[]; nota_base?: number; ajustes_itens?: number; deducoes_erros_graves?: number; nota_bruta?: number; ficha_caso?: { fator_principal: string; fatores_risco: string[]; fatores_protecao: string[] }; acertos?: string[]; melhorias?: string[]; linha_evolucao?: { fala: string; observacao: string }[]; motivo_encerramento?: string; encerramento_forcado?: boolean; ocorrencias_graves?: number } };
...
export default async function SessionPage({ params }: { params: Promise<{ sessionId: string }> }) {
  try {
    ...
    const { data, error } = await supabase.from("training_sessions").select("id, difficulty, status, image_path, public_briefing, started_at, active_elapsed_ms, active_activity, active_started_at").eq("id", sessionId).maybeSingle();
    ...
    const initialSilenceAvailable = !allTurns.some((turn) => turn.speaker === "ALUNO" || turn.event_metadata?.event === "SILENCIO_INICIAL");
    ...
    return <AppShell backHref="/app" backLabel="Minhas sessões" tone="immersive">
      <div className="dashboard">
        ...
        {!terminal && <><SessionExperience sessionId={session.id} briefing={briefing} locationUrl={locationSigned?.data?.signedUrl ?? null} lastCharacterTurn={openingCharacterTurn ? { id: openingCharacterTurn.id, pending: true, content: openingCharacterTurn.content } : null} pendingCharacterTurn={pendingCharacterTurn && !openingCharacterTurn ? { id: pendingCharacterTurn.id, pending: true, content: pendingCharacterTurn.content } : null} difficulty={session.difficulty} startedAt={session.started_at} remainingAtLoad={remainingSessionMs(session.difficulty, session)} activityAtLoad={session.active_activity} initialSilenceAvailable={initialSilenceAvailable} /><form action={endTrainingSession} className="manual-end-form"><input type="hidden" name="sessionId" value={session.id} /><button type="submit">Encerrar e avaliar</button><p>Encerra a simulação e calcula somente as condutas já registradas.</p></form></>}
        ...
        {evaluation ? <section className="panel result-panel"><div className="eyebrow">Avaliação automática</div><h2>Simulação concluída</h2><p className="result-score">{evaluation.final_score.toFixed(1)} <small>/ 10</small></p><p>Desfecho: <strong>{evaluation.result === "EXITO" ? "saída digna aceita" : "simulação encerrada"}</strong>. Cobertura observável: {evaluation.calculation?.cobertura?.avaliados ?? 0}/{evaluation.calculation?.cobertura?.total ?? 17} itens.</p><p className="notice">{evaluation.calculation?.rotulo ?? "Estimativa didática; não é nota oficial do curso."}</p><p className={evaluation.calculation?.encerramento_forcado ? "evaluation-zero" : ""}><strong>{evaluation.calculation?.motivo_encerramento ?? "SIMULAÇÃO ENCERRADA"}</strong>{evaluation.calculation?.ocorrencias_graves ? " — " + evaluation.calculation.ocorrencias_graves + " ocorrências graves registradas." : ""}</p>{evaluation.calculation?.ficha_caso && <details open><summary>Ficha revelada do caso</summary><p><strong>Fator principal:</strong> {evaluation.calculation.ficha_caso.fator_principal}</p><p><strong>Fatores de risco:</strong> {evaluation.calculation.ficha_caso.fatores_risco.join("; ")}</p><p><strong>Fatores de proteção:</strong> {evaluation.calculation.ficha_caso.fatores_protecao.join("; ")}</p></details>}{evaluation.calculation?.aviso_parcial && <p className="notice">{evaluation.calculation.aviso_parcial}</p>}{evaluation.calculation?.itens && <details open className="evaluation-items"><summary>Ficha de avaliação item a item</summary><ul>{evaluation.calculation.itens.map((item) => <li key={item.id} className={item.estado === "feito" || item.estado === "adequado" || item.estado === "encontrou_explorou" || item.estado === "encontrou_isolou" ? "evaluation-item-positive" : item.estado === "parcial" ? "evaluation-item-warning" : "evaluation-item-negative"}><strong>{item.titulo}</strong><span> {item.ajuste >= 0 ? "+" : ""}{item.ajuste.toFixed(1)} — {item.estado.replaceAll("_", " ")}</span><p>{item.evidencia}</p></li>)}</ul></details>}<section><h3>Acertos mais relevantes</h3><ul>{evaluation.calculation?.acertos?.map((item) => <li key={item}>{item}</li>)}</ul><h3>Ajustes sugeridos</h3><ul>{evaluation.calculation?.melhorias?.map((item) => <li key={item}>{item}</li>)}</ul><h3>Linha de evolução</h3><ul>{evaluation.calculation?.linha_evolucao?.map((item, index) => <li key={index}><strong>Você:</strong> {item.fala} - <em>{item.observacao}</em></li>)}</ul></section><Link className="button-link" href="/app">Voltar às sessões</Link></section> : terminal ? <section className="panel"><h2>Simulação encerrada</h2><p className="panel-subtitle">O relatório não está disponível. Atualize a página ou avise o administrador.</p></section> : <section className="panel"><h2>Enviar por texto</h2><p className="panel-subtitle">Use este campo se preferir escrever ou se o microfone não estiver disponível.</p><TextTurnForm sessionId={session.id} /></section>}
        {evaluation && <ExportSessionPdf title={briefing.titulo} difficulty={difficultyLabel[session.difficulty] ?? session.difficulty} transcript={turns} evaluation={{ result: evaluation.result, finalScore: evaluation.final_score, calculation: evaluation.calculation }} />}
