...
type Evaluation = { result: string; final_score: number; partial: boolean; calculation: { cobertura?: { avaliados: number; total: number; percentual: number }; rotulo?: string; aviso_parcial?: string | null; itens?: EvaluationItem[] } };
  ...
  try {
    ...
    return <AppShell backHref="/app" backLabel="Minhas sessões" tone="immersive">
      <div className="dashboard">
        ...
        {evaluation ? <section className="panel result-panel"><div className="eyebrow">Avaliação automática</div><h2>Simulação concluída</h2><p className="result-score">{evaluation.final_score.toFixed(1)} <small>/ 10</small></p><p>Desfecho: <strong>{evaluation.result === "EXITO" ? "saída digna aceita" : "simulação encerrada"}</strong>. Cobertura observável: {evaluation.calculation?.cobertura?.avaliados ?? 0}/{evaluation.calculation?.cobertura?.total ?? 17} itens.</p><p className="notice">{evaluation.calculation?.rotulo ?? "Estimativa didática; não é nota oficial do curso."}</p>{evaluation.calculation?.aviso_parcial && <p className="notice">{evaluation.calculation.aviso_parcial}</p>}{evaluation.calculation?.itens && <details><summary>Ficha de avaliação item a item</summary><ul>{evaluation.calculation.itens.map((item) => <li key={item.id}><strong>{item.titulo}</strong>: {item.ajuste >= 0 ? "+" : ""}{item.ajuste.toFixed(1)} — {item.estado.replaceAll("_", " ")}</li>)}</ul></details>}<Link className="button-link" href="/app">Voltar às sessões</Link></section> : terminal ? <section className="panel"><h2>Simulação encerrada</h2><p className="panel-subtitle">O relatório não está disponível. Atualize a página ou avise o administrador.</p></section> : <section className="panel"><h2>Enviar por texto</h2><p className="panel-subtitle">Use este campo se preferir escrever ou se o microfone não estiver disponível.</p><TextTurnForm sessionId={session.id} /></section>}
