...
export async function createTrainingSession(formData: FormData) {
  ...
  const { user } = await requireApprovedUser(); const { internalCase, publicBriefing } = createSessionCase(difficulty.data); const admin = createSupabaseAdminClient();
  ...
  try {
    const path = `${session.id}/pov.webp`; const bytes = await generatePovImage(publicBriefing);
  ...
  } catch { /* A conversa textual permanece disponível se o provedor de imagem falhar. */ }
  redirect(`/app/sessions/${session.id}`);
}

export async function sendTrainingTurn(formData: FormData) {
  const sessionId = String(formData.get("sessionId") ?? ""); const content = String(formData.get("content") ?? "").trim();
  ...
  const transcript = (previous ?? []).map((turn) => ({ speaker: turn.speaker as "ALUNO" | "PERSONAGEM" | "NARRADOR" | "SISTEMA", content: turn.content })); const sequence = (previous?.at(-1)?.sequence_number ?? 0) + 1;
